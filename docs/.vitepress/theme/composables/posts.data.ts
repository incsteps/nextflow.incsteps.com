...
async function load() {
  ...
  return fs
    ...
    .filter(file => isFile(file as string, dir))
    .map(file => getPost(file as string, dir))
...
function isFile(file: string, postDir: string): boolean {
  const fullPath = path.join(postDir, file)
  const stats = fs.statSync(fullPath)
  console.error(`${fullPath} ${stats.isDirectory()}`)
